...
export function addColumn(
    ...
    for (let row = 0; row < map.height; row++) {
        ...
        if (col > 0 && col < map.width && map.map[index - 1] === map.map[index]) {
            ...
            tr.setNodeMarkup(tr.mapping.map(tableStart + pos), undefined, addColSpan(cell.attrs, col - map.colCount(pos)))
 ...
 */
export function addColumnBefore(state: EditorState, dispatch: DispatchFunction | undefined): boolean {
    if (!isInTable(state)) {
        return false
    }
 ...
 */
export function addColumnAfter(state: EditorState, dispatch: DispatchFunction | undefined): boolean {
    if (!isInTable(state)) {
        return false
    }
