...
export function injectControllers({ schema, getMap, table: oldTable }: InjectControllersProps): ProsemirrorNode {
...
export function createControllerEvents({ view, findTable }: { view: EditorView; findTable: FindTable }): Events {
    return {
        onClick: (event) => {
...
function onlyTableFound<Extra extends unknown[]>(func: (view: EditorView, table: FindProsemirrorNodeResult, ...extra: Extra) => void) {
...
const selectRow = onlyTableFound((view: EditorView, table: FindProsemirrorNodeResult, index: number) => {
...
const selectColumn = onlyTableFound((view: EditorView, table: FindProsemirrorNodeResult, index: number) => {
    const map = TableMap.get(table.node)
...
export function getCellAxisByCoords(view: EditorView, coords: { left: number; top: number }): CellAxis | null {
