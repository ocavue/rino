...
function buildInsertButtonAttrs(type: TriggerAreaType, triggerRect: DOMRect, editorDom: Element, cellAxis: CellAxis): InsertButtonAttrs {
...
function showButton(trigger: HTMLElement, findTable: FindTable, type: TriggerAreaType, axis: CellAxis, view: EditorView): void {
...
const TableInsertButtonTrigger = ({ view, findTable }: { view: EditorView; findTable: FindTable }): HTMLElement[] => {
    return [TriggerArea({ view, findTable, isTopLeft: true }), TriggerArea({ view, findTable, isTopLeft: false })]
}
