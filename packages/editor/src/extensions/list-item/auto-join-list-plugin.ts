import { CreateExtensionPlugin, ProsemirrorNode, Transaction } from "@remirror/core"
import { canJoin } from "@remirror/pm/transform"

import { isListItemNode } from "./list-utils"

export function createAutoJoinListPlugin(): CreateExtensionPlugin {
    return {
                ...
                for (let index = $from.indexAfter(depth), pos = $from.after(depth + 1); pos <= to; ++index) {
                    ...
                    if (index && !joinDpeths.has(pos)) {
                        ...
                        if (before.type === after.type) {
                            const joinDepth = isListJoinable(before, after)
                            if (joinDepth) {
...
function isListJoinable(before: ProsemirrorNode | null, after: ProsemirrorNode | null): number {
    let joinDepth = 0
