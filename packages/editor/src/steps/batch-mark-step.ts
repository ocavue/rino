import { Fragment, Mark, Node, NodeRange, Schema, Slice } from "@remirror/pm/model"
import { Mappable, Step, StepResult } from "@remirror/pm/transform"
...
export class BatchSetMarkStep extends Step {
    ...
    apply(doc: Node): StepResult {
        ...
        const fragment = mapFragment(oldSlice.content, start, parent, (inlineNode: Node, parent: Node, nodeLo: number) => {
            node = inlineNode
        ...
        return StepResult.fromReplace(doc, start, to, new Slice(fragment, oldSlice.openStart, oldSlice.openEnd))
    }
    ...
    toJSON(): any {
        const chunks = this.chunks.map((chunk) => [chunk[0], chunk[1], chunk[2].map((mark: Mark) => mark.toJSON())])
